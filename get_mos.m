...
for j=1:2
    ...
    ajc_v=filtfilt(b,a,struc.Misc.(jc_lab)(:,3)');
    temp_tab=[];
    ...
    mos_struc.(side{j}).XcoM_AP=xcom_ap;
    mos_struc.(side{j}).XcoM_ML=xcom_ml;
    ...
    heel_ap=filtfilt(b,a,-struc.Markers.(heel_lab)(:,2)');
    toe_ap=filtfilt(b,a,-struc.Markers.(toe_lab)(:,2)');
    if j==1
        vmh_ml=filtfilt(b,a,struc.Markers.(vmh_lab)(:,1)');
    else
        vmh_ml=filtfilt(b,a,-struc.Markers.(vmh_lab)(:,1)');
    end
    ...
    for cy=1:nbc
        ...
        if isempty(TOcy)
            ...
            mos_struc.(side{j}).MoS_AP_Heel.Cycles{cy,1}=temp_heel;
            mos_struc.(side{j}).MoS_AP_Toe.Cycles{cy,1}=temp_toe;
            mos_struc.(side{j}).MoS_ML.Cycles{cy,1}=temp_vmh;
    ...
    trocanter_height=struc.Misc.(leg_lab)-struc.Misc.(atd_lab);
    mos_struc.(side{j}).TrocanterHeight=trocanter_height;

    mos_struc.(side{j}).MoS_AP_Heel.NormalizedMean=mean(mos_ap_heel_temp,1)/trocanter_height;
    mos_struc.(side{j}).MoS_AP_Toe.NormalizedMean=mean(mos_ap_toe_temp,1)/trocanter_height;
    mos_struc.(side{j}).MoS_ML.NormalizedMean=mean(mos_ml_temp,1)/trocanter_height;

    mos_struc.(side{j}).MoS_AP_Heel.NormalizedStd=std(mos_ap_heel_temp,0,1)/trocanter_height;
    mos_struc.(side{j}).MoS_AP_Toe.NormalizedStd=std(mos_ap_toe_temp,0,1)/trocanter_height;
    mos_struc.(side{j}).MoS_ML.NormalizedStd=std(mos_ml_temp,0,1)/trocanter_height;
